...
export function getBotDecision(player: Player, square: Square): 'buy' | 'skip' {
  ...
  // Don't buy if we can't afford it or it would leave us with less than 50 coins
  if (player.coins < property.price + 50) {
    return 'skip';
  }
...
export function getBotMarketDecision(player: Player): Item | null {
  const availableItems = generateMarketItems();
  
  // Don't buy if we have less than 150 coins
  if (player.coins < 150) return null;
  ...
  for (const item of availableItems) {
    ...
    if (player.coins < item.value + 50) continue;
